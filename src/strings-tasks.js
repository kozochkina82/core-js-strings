...
function isPalindrome(str) {
  const normalString = str.replace(/[^A-Z0-9]/gi, '').toLowerCase();
  const checkPalindrome = normalString.split('').reverse().join('');
  if (normalString === checkPalindrome) {
    return true;
  }
  return false;
}
...
function invertCase(str) {
  const newStr = str.replace(/[A-Za-z]/g, function aChar(char) {
...
function extractNameFromTemplate(value) {
  const valueNew = value.slice(0, value.length - 1);
  const name = valueNew.slice(7);
  return name;
}
...
function encodeToRot13(str) {
  ...
  const myAlpha = 'nopqrstuvwxyzabcdefghijklmNOPQRSTUVWXYZABCDEFGHIJKLM';
  return str.replace(
    /[a-z]/gi,
    (letter) => myAlpha[originalAlpha.indexOf(letter)]
  );
}
